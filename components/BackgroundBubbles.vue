<template>
  <div class="bubble-container pointer-events-none fixed inset-0 -z-10">
    <div
...
onMounted(async () => {
  ...
  visibleBubbles.value.forEach(bubble => {
    ...
    if (!el) return
    const appearStart =
      bubble.appearScrollPoint *
      (document.documentElement.scrollHeight - window.innerHeight)
    const popEnd =
      bubble.popScrollPoint *
      (document.documentElement.scrollHeight - window.innerHeight)
    ...
    if (bubble.appearScrollPoint > 0) {
      ...
      tl.to(el, { scale: 1, duration: 0.2, ease: 'elastic.out(1, 0.5)' })
    } else {
    ...
    tl.to(el, {
      scale: 1.3,
      opacity: 0,
      rotate: bubble.rotation + (Math.random() - 0.5) * 20,
      duration: 0.25,
      ease: 'back.in(2)',
    })
  })
...
const visibleBubbles = computed(() => {
  ...
  return bubbles.value.filter(
    b => progress >= b.appearScrollPoint && progress <= b.popScrollPoint
  )
})
</script>
